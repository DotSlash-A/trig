...
from sympy import (
    ...
    SympifyError,
    symbols,
)  # Renamed Poly to SympyPoly to avoid clash
...
# def parse_polynomial_string(
#     poly_str: str, var_symbol: str = "x"
...
#             raise ValueError(
#                 "Expression is not a polynomial in the specified variable."
#             )
...
def polynomial_to_string(
    coeffs: List[Union[float, complex]], var_symbol: str = "x"
) -> str:
    ...
    while (
        first_non_zero_idx < len(coeffs) - 1 and abs(coeffs[first_non_zero_idx]) < 1e-9
    ):
    ...
    for i, coeff_val in enumerate(coeffs_cleaned):
        ...
        if is_complex_coeff:
            ...
            if has_imag:
                if (
                    has_real and imag_part > 0
                ):  # Add plus if real part exists and imag is positive
                    coeff_str += "+"
                ...
                if abs(abs(imag_part) - 1.0) < 1e-9:  # if imag_part is 1 or -1
                    coeff_str += (
                        "j" if imag_part > 0 else ("-j" if not has_real else "j")
                    )  # handle -j if no real part
                    if imag_part < 0 and not has_real:
                        coeff_str = "-j"  # Ensure -j if purely -1j
                else:
                    coeff_str += f"{imag_part:.4g}".rstrip("0").rstrip(".") + "j"

            if (
                has_real or has_imag
            ):  # Need parentheses if complex and not just a number
        ...
        if power > 0:
            if (
                coeff_str == "" and abs(abs(real_part) - 1.0) < 1e-9
            ):  # Handle "x" not "1x"
                pass  # No numerical coefficient string, sign handled
            elif (
                coeff_str
                and term_str
                and not term_str.endswith(" ")
                and not term_str.endswith(")")
            ):
    ...
    result = "".join(
        filter(None, terms)
    )  # Filter out empty strings from skipped 1 coefficients
    if not result:
        return "0"
...
def evaluate_polynomial(
    coeffs: List[Union[float, complex]], x_val: Union[float, complex]
) -> Union[float, complex]:
...
def polynomial_division(
    dividend_coeffs: List[Union[float, complex]],
    divisor_coeffs: List[Union[float, complex]],
) -> Tuple[List[Union[float, complex]], List[Union[float, complex]]]:
    """
    ...
    if deg_dividend < deg_divisor:
        return [complex(0.0)], list(
            dividend_coeffs
        )  # Quotient is 0, remainder is dividend

    quotient_coeffs: List[Union[float, complex]] = [complex(0.0)] * (
        deg_dividend - deg_divisor + 1
    )
    # Make a mutable copy for remainder that supports complex numbers
    current_remainder_coeffs: List[Union[float, complex]] = [
        complex(c) for c in dividend_coeffs
    ]

    lead_divisor_coeff = divisor_coeffs[0]
    if (
        abs(lead_divisor_coeff) < 1e-9
    ):  # Should be caught by initial check but good to have
    ...
    while (
        first_non_zero_rem_idx < len(final_remainder)
        and abs(final_remainder[first_non_zero_rem_idx]) < 1e-9
    ):
...
def synthetic_division(
    dividend_coeffs: List[Union[float, complex]], a_val: Union[float, complex]
) -> Tuple[List[Union[float, complex]], Union[float, complex]]:
    """
    ...
    if n == 1:  # Dividend is a constant
        return [complex(0.0)], dividend_coeffs[
            0
        ]  # Quotient is 0, remainder is the constant
    ...
    if abs(a) < 1e-9:  # Not a quadratic equation if a is zero
        ...
        if abs(b) < 1e-9:  # 0x = -c
            if abs(c) < 1e-9:
                ...
                solution_payload["description"] = (
                    f"Contradiction (0 = {-c}), no solution."
                )
...
def relation_roots_coeffs_quadratic(
    ...
    return {
        "alpha_plus_beta": (
            str(sum_roots) if isinstance(sum_roots, complex) else sum_roots
        ),
        "alpha_beta": (
            str(prod_roots) if isinstance(prod_roots, complex) else prod_roots
        ),
        "verification_note": "These are theoretical values based on coefficients.",
    }


def relation_roots_coeffs_cubic(
    coeffs: List[Union[float, complex]],
) -> Optional[Dict[str, Any]]:
    ...
    prod_roots = -d / a
    return {
        "alpha_plus_beta_plus_gamma": (
            str(sum_roots) if isinstance(sum_roots, complex) else sum_roots
        ),
        "alpha_beta_plus_beta_gamma_plus_gamma_alpha": (
            str(sum_prod_pairs)
            if isinstance(sum_prod_pairs, complex)
            else sum_prod_pairs
        ),
        "alpha_beta_gamma": (
            str(prod_roots) if isinstance(prod_roots, complex) else prod_roots
...
def find_rational_roots(coeffs: List[Union[float, complex]]) -> List[float]:
    ...
    if any(isinstance(c, complex) and abs(c.imag) > 1e-9 for c in coeffs):
        print(
            "Warning: Rational Root Theorem is best suited for polynomials with real (ideally integer) coefficients. Results may be unreliable for complex coefficients."
        )
    ...
    for c_real in real_coeffs_for_rrt:
        if abs(c_real - round(c_real)) > 1e-9:
            print(
                f"Warning: Coefficient {c_real} is not an integer. Rational Root Theorem strictly applies to integer coefficients."
            )
    ...
    def get_divisors(n: int) -> List[int]:
        n_abs = abs(n)
        if n_abs == 0:
            return [0]  # Special case for a0=0, p can be 0
        divs = set()
...
def find_all_roots_numeric(
    coeffs: List[Union[float, complex]], attempts: int = 10
) -> List[str]:
    """
    ...
    try:
        ...
        return [
            str(r.evalf(15)) for r in roots_found
        ]  # evalf for consistent string format
...
def multiply_polynomials(
    poly1_coeffs: List[Union[float, complex]], poly2_coeffs: List[Union[float, complex]]
) -> List[Union[float, complex]]:
