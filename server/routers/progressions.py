...
import numpy as np
import sympy
router = APIRouter(prefix="/progressions", tags=["Integration"])
@router.get("/test")
async def test():
...
class MiddleTermInput(BaseModel): # Renamed for clarity
    ...
    a: float = Field(..., description="First term of the arithmetic progression")
    d: float = Field(..., description="Common difference of the arithmetic progression")
    last_term: float = Field(..., description="Value of the last term in the arithmetic progression") # Renamed n to last_term
...
async def middle_term(request: MiddleTermInput): # Use the renamed model
    ...
    if n_float <= 0 or n_float != int(n_float):
         raise HTTPException(status_code=400, detail="Invalid progression parameters leading to non-positive or non-integer number of terms.")
