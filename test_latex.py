...
def evaluate_expression(tokens: List[str], var_value: float, variable: str) -> float:
    ...
    def apply_op():
        ...
        try:
            if op == "unary_minus":
                ...
                if op == "sin":
                    ...
                    if abs(tan_val) < 1e-15:  # Check if tan is near zero
                        ...
                        abs(math.cos(val)) < 1e-15
                    ):  # Check if cos is near zero (tan undefined)
                    ...
                    try:
                        res = math.exp(val)
                        values.append(
                            res if abs(res) < 1e300 else (INF if res > 0 else NEG_INF)
                        )
            ...
            logger.error(
                f"Numerical Eval Error: Not enough operands for operator '{op}'. Values: {values}, Ops: {ops}"
            )
    ...
    for i, token in enumerate(tokens):
        ...
        if is_number(token):
            ...
            if var_value is None:
                ...
                logger.error(
                    "Numerical Eval Error: Variable value is None during evaluation."
                )
                values.append(NAN)
                while ops:
                    ops.pop()
            ...
            while ops and ops[-1] != "(":
                ...
                if not ops and values and math.isnan(values[-1]):
                    break
            if not ops:  # Error occurred in apply_op or mismatched parens
                if not values or not math.isnan(
                    values[-1]
                ):  # Avoid adding step if already handled
                    logger.error(
                        "Numerical Eval Error: Mismatched parentheses or error during application."
                    )
            ...
            if values and math.isnan(values[-1]) and not ops:
                break
...
class LimitCalculator:
    def __init__(self, expression: str, variable: str, tending_to_str: str):
        ...
        try:
            ...
            if t_str in ["inf", "infinity", "oo", "∞"]:
                ...
                self.sympy_limit_point = sympy.sympify(
                    parsed_limit_point, locals={"pi": sympy.pi, "e": sympy.E}
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not evaluate limit point {self.sympy_limit_point} numerically."
                    )
                    self.sympy_limit_point_val = (
                        None  # Cannot perform numerical checks easily
                    )
        ...
        try:
            ...
            self.fallback_tokens = tokenize(
                self.original_expression, self.variable_str
            )  # Use original expression for fallback tokenizer
            ...
            if self.sympy_limit_point_val is not None:
                ...
                self.numerical_limit_point = self._parse_limit_point_numerical(
                    tending_to_str
                )
    ...
    def _parse_limit_point_numerical(self, limit_point_str: str) -> float:
        ...
        if t_str in ["inf", "infinity", "∞"]:
            ...
            try:
                ...
                val = eval(
                    t_str, {"__builtins__": None}, {"pi": PI, "e": E, "sqrt": math.sqrt}
                )
                ...
                logger.error(
                    f"Failed to parse numerical limit point '{limit_point_str}': {e}"
                )
    ...
    def _format_sympy_result(self, sympy_res) -> str:
        ...
        """Formats sympy result into a concise string representation."""
        if sympy_res is sympy.oo:
            ...
            # Return a simplified representation for unevaluated limits in the final result
            return f"Limit({sympy.latex(sympy_res.args[0])}, {sympy.latex(sympy_res.args[1])}, {sympy.latex(sympy_res.args[2])})"
            ...
            try:
                ...
                if rational_approx.q == 1:
                    ...
                    # Keep float representation if denominator is large after approximation
                    if rational_approx.q > 100:
                        return f"{float(sympy_res):.6g}"  # Use float formatting
            ...
            # General sympy expression, convert to simple string if possible, otherwise LaTeX
            try:
                # Use str() for a potentially simpler output for the final result
                return str(sympy_res)
            except Exception:
                # Fallback to latex if str fails or is too complex
                return f"$${sympy.latex(sympy_res)}$$"
    ...
    def _safe_numerical_evaluate(self, value: float) -> float:
        ...
        try:
            ...
            eval_result = evaluate_expression(
                self.fallback_tokens, value, self.variable_str
            )

            # Check for NaN or Inf explicitly from the numerical evaluator
            if isinstance(eval_result, float) and (
                math.isnan(eval_result) or math.isinf(eval_result)
            ):
                return eval_result  # Return NaN, INF, NEG_INF as is
            elif not isinstance(eval_result, (int, float)):
                logger.error(
                    f"Numerical evaluation returned non-numeric type: {type(eval_result)}"
                )
                return NAN
            ...
            logger.exception(
                f"Unexpected error during numerical evaluation at value {value}: {e}"
            )
            return NAN

    def calculate(self) -> Dict[str, Any]:
        # Helper to format expressions for steps using LaTeX
        def format_expr_latex(expr):
            try:
                return f"$${sympy.latex(expr)}$$"
            except Exception as e:
                logger.error(
                    f"Error generating LaTeX for expression: {expr}, Error: {e}"
                )
                # Fallback to pretty print or simple string if LaTeX fails
                try:
        ...
        self._add_step(
            "Initial Expression",
            f"Find the limit of f({self.variable_str}) = {format_expr_latex(self.sympy_expr)} as {self.variable_str} → {self.tending_to_str}",
        )
        ...
        try:
            ...
            self._add_step(
                ...
                f"Substitute {self.variable_str} = {self.tending_to_str} into the expression.",
            )
            try:
                # Use subs for substitution, then evaluate if possible
                sub_result = current_expr.subs(self.sympy_var, self.sympy_limit_point)
                # Evaluate numerical results if possible after substitution
                if hasattr(sub_result, "evalf"):
                    try:
                        eval_result = sub_result.evalf()
                        # Check if evalf() returned a numerical type or infinity
                        if isinstance(
                            eval_result, (sympy.Float, sympy.Integer)
                        ) or eval_result in [sympy.oo, -sympy.oo]:
                            sub_result = (
                                eval_result  # Use the evaluated numerical result
                            )
                        # Handle specific cases like sqrt(-1) -> I
                        elif eval_result.is_imaginary:
                            sub_result = eval_result
                    except (TypeError, ValueError):
                        pass  # Keep symbolic result if evalf fails

                self._add_step(
                    "Substitution Result", f"Result: {format_expr_latex(sub_result)}"
                )

                # Check if substitution yielded a direct answer (finite, oo, -oo)
                is_indeterminate = (
                    sub_result is sympy.nan
                    or sub_result is sympy.zoo  # complex infinity is indeterminate
                    or (
                        hasattr(sub_result, "is_infinite")
                        and sub_result.is_infinite
                        and sub_result not in [sympy.oo, -sympy.oo]
                    )  # Other infinities
                    or (
                        hasattr(sub_result, "has")
                        and sub_result.has(
                            sympy.oo, -sympy.oo, sympy.zoo, sympy.nan, sympy.I
                        )
                    )  # Check for oo, nan, or I within the expression
                )
                # Handle 0/0 or oo/oo cases explicitly
                num, den = current_expr.as_numer_denom()
                num_limit_eval = sympy.limit(
                    num, self.sympy_var, self.sympy_limit_point
                )
                den_limit_eval = sympy.limit(
                    den, self.sympy_var, self.sympy_limit_point
                )

                is_zero_over_zero = num_limit_eval == 0 and den_limit_eval == 0
                is_inf_over_inf = (
                    hasattr(num_limit_eval, "is_infinite")
                    and num_limit_eval.is_infinite
                    and hasattr(den_limit_eval, "is_infinite")
                    and den_limit_eval.is_infinite
                )

                indeterminate_form = None
                if is_zero_over_zero:
                    indeterminate_form = "0/0"
                    is_indeterminate = True  # Ensure this flags as indeterminate
                elif is_inf_over_inf:
                    # Check sign for oo/oo, -oo/oo etc.
                    if (num_limit_eval == sympy.oo and den_limit_eval == sympy.oo) or (
                        num_limit_eval == -sympy.oo and den_limit_eval == -sympy.oo
                    ):
                        indeterminate_form = "∞/∞"
                    elif (
                        num_limit_eval == sympy.oo and den_limit_eval == -sympy.oo
                    ) or (num_limit_eval == -sympy.oo and den_limit_eval == sympy.oo):
                        indeterminate_form = "-∞/∞"  # Or similar
                    else:  # Mixed infinities or complex infinity
                        indeterminate_form = "∞/∞ type"
                    is_indeterminate = True  # Ensure this flags as indeterminate

                if not is_indeterminate and (
                    hasattr(sub_result, "is_finite") and sub_result.is_finite
                ):
                    final_result_obj = sub_result
                    limit_calculated = True
                    self._add_step(
                        "Conclusion",
                        f"Direct substitution yields a determinate value: {self._format_sympy_result(final_result_obj)}",
                    )
                elif sub_result in [
                    sympy.oo,
                    -sympy.oo,
                ]:  # Handle direct substitution resulting in infinity
                    final_result_obj = sub_result
                    limit_calculated = True
                    self._add_step(
                        "Conclusion",
                        f"Direct substitution yields: {self._format_sympy_result(final_result_obj)}",
                    )
                else:
                    # Report indeterminate form
                    if indeterminate_form:
                        self._add_step(
                            "Indeterminate Form",
                            f"Substitution results in an indeterminate form of type {indeterminate_form}.",
                        )
                    else:
                        # General indeterminate case
                        self._add_step(
                            "Indeterminate Form",
                            f"Substitution results in an indeterminate form ({format_expr_latex(sub_result)}). Further steps needed.",
                        )
            ...
            if not limit_calculated:
                self._add_step(
                    "Attempt Simplification", "Try simplifying the expression."
                )
                try:
                    # Use cancel for rational functions, simplify for general expressions
                    original_expr_for_comparison = current_expr
                    if current_expr.is_rational_function(self.sympy_var):
                        simplified_expr = sympy.cancel(current_expr)
                        simplification_method = "sympy.cancel"
                    else:
                        # Use a more general simplification
                        simplified_expr = sympy.simplify(current_expr)
                        # Sometimes trigsimp helps where simplify doesn't
                        if simplified_expr == current_expr and (
                            current_expr.has(sympy.sin) or current_expr.has(sympy.cos)
                        ):
                            simplified_expr = sympy.trigsimp(current_expr)
                            if simplified_expr != current_expr:
                                simplification_method = "sympy.trigsimp"
                            else:
                                simplification_method = (
                                    "sympy.simplify"  # Revert if trigsimp didn't help
                                )
                        else:
                            simplification_method = "sympy.simplify"

                    if simplified_expr != original_expr_for_comparison:
                        self._add_step(
                            "Simplification Result",
                            f"Using {simplification_method}: {format_expr_latex(simplified_expr)}",
                        )
                        current_expr = simplified_expr  # Update current expression

                        # Try substitution again on the simplified expression
                        self._add_step(
                            "Attempt Substitution (Simplified)",
                            f"Substitute {self.variable_str} = {self.tending_to_str} into the simplified expression.",
                        )
                        sub_result_simplified = current_expr.subs(
                            self.sympy_var, self.sympy_limit_point
                        )
                        # Evaluate numerical results if possible
                        if hasattr(sub_result_simplified, "evalf"):
                            try:
                                eval_result_simplified = sub_result_simplified.evalf()
                                if isinstance(
                                    eval_result_simplified, (sympy.Float, sympy.Integer)
                                ) or eval_result_simplified in [sympy.oo, -sympy.oo]:
                                    sub_result_simplified = eval_result_simplified
                                elif eval_result_simplified.is_imaginary:
                                    sub_result_simplified = eval_result_simplified
                            except (TypeError, ValueError):
                                pass  # Keep symbolic result if evalf fails

                        self._add_step(
                            "Substitution Result (Simplified)",
                            f"Result: {format_expr_latex(sub_result_simplified)}",
                        )

                        is_indeterminate_simplified = (
                            sub_result_simplified is sympy.nan
                            or sub_result_simplified is sympy.zoo
                            or (
                                hasattr(sub_result_simplified, "is_infinite")
                                and sub_result_simplified.is_infinite
                                and sub_result_simplified not in [sympy.oo, -sympy.oo]
                            )
                            or (
                                hasattr(sub_result_simplified, "has")
                                and sub_result_simplified.has(
                                    sympy.oo, -sympy.oo, sympy.zoo, sympy.nan, sympy.I
                                )
                            )
                        )
                        # Check form again for L'Hopital's
                        num_s, den_s = current_expr.as_numer_denom()
                        num_limit_s = sympy.limit(
                            num_s, self.sympy_var, self.sympy_limit_point
                        )
                        den_limit_s = sympy.limit(
                            den_s, self.sympy_var, self.sympy_limit_point
                        )
                        is_zero_over_zero_s = num_limit_s == 0 and den_limit_s == 0
                        is_inf_over_inf_s = (
                            hasattr(num_limit_s, "is_infinite")
                            and num_limit_s.is_infinite
                            and hasattr(den_limit_s, "is_infinite")
                            and den_limit_s.is_infinite
                        )

                        if is_zero_over_zero_s or is_inf_over_inf_s:
                            is_indeterminate_simplified = True  # Ensure flag is set

                        if not is_indeterminate_simplified and (
                            hasattr(sub_result_simplified, "is_finite")
                            and sub_result_simplified.is_finite
                        ):
                            final_result_obj = sub_result_simplified
                            limit_calculated = True
                            self._add_step(
                                "Conclusion",
                                f"Substitution into simplified expression yields: {self._format_sympy_result(final_result_obj)}",
                            )
                        elif sub_result_simplified in [
                            sympy.oo,
                            -sympy.oo,
                        ]:  # Handle infinity result after simplification
                            final_result_obj = sub_result_simplified
                            limit_calculated = True
                            self._add_step(
                                "Conclusion",
                            ...
                            # Report indeterminate form after simplification
                            form_type_s = ""
                            if is_zero_over_zero_s:
                                form_type_s = "0/0"
                            elif is_inf_over_inf_s:
                                form_type_s = "∞/∞ type"

                            if form_type_s:
                                self._add_step(
                                    "Indeterminate Form (Simplified)",
                                    f"Simplified expression still results in an indeterminate form ({form_type_s}).",
                                )
                            else:
                                self._add_step(
                                    "Indeterminate Form (Simplified)",
                                    f"Substitution results in an indeterminate form ({format_expr_latex(sub_result_simplified)}).",
                                )

                    else:
                        self._add_step(
                            "Simplification Result",
                            "Expression could not be simplified further.",
                        )

                except Exception as e:
                    self._add_step(
                        "Simplification Error", f"Error during simplification: {e}"
                    )
            ...
            if not limit_calculated:
                ...
                num, den = current_expr.as_numer_denom()
                # Re-evaluate limits for the current (potentially simplified) expression
                ...
                is_lhopital_applicable = (num_limit == 0 and den_limit == 0) or (
                    hasattr(num_limit, "is_infinite")
                    and num_limit.is_infinite
                    and hasattr(den_limit, "is_infinite")
                    and den_limit.is_infinite
                )

                if is_lhopital_applicable:
                    form_type = "0/0" if num_limit == 0 else "∞/∞"
                    self._add_step(
                        "Attempt L'Hôpital's Rule",
                        f"Indeterminate form ({form_type}) detected. Applying L'Hôpital's Rule.",
                    )
                    try:
                        ...
                        self._add_step(
                            "Derivatives",
                            f"Derivative of numerator: {format_expr_latex(num_diff)}\nDerivative of denominator: {format_expr_latex(den_diff)}",
                        )

                        if den_diff == 0:
                            # This case is tricky. It might mean the limit is infinite or L'Hopital's doesn't apply cleanly.
                            # Let sympy.limit handle it in the next step.
                            self._add_step(
                                "L'Hôpital's Rule Note",
                                "Denominator derivative is zero. Proceeding to evaluate limit of ratio of derivatives.",
                            )
                            # We still form the expression and let sympy.limit try
                            lhopital_expr = (
                                num_diff / den_diff
                            )  # This might create DivisionByZero object
                        else:
                            lhopital_expr = num_diff / den_diff
                            # Simplify the L'Hopital expression before taking the limit
                            simplified_lhopital_expr = sympy.simplify(lhopital_expr)
                            if simplified_lhopital_expr != lhopital_expr:
                                self._add_step(
                                    "L'Hôpital's Expression (Simplified)",
                                    f"Simplified ratio of derivatives: {format_expr_latex(simplified_lhopital_expr)}",
                                )
                                lhopital_expr = simplified_lhopital_expr
                            else:
                                self._add_step(
                                    "L'Hôpital's Expression",
                                    f"Ratio of derivatives: {format_expr_latex(lhopital_expr)}",
                                )

                        # Calculate limit of the new expression
                        self._add_step(
                            "Calculate Limit (L'Hôpital)",
                            f"Find limit of {format_expr_latex(lhopital_expr)} as {self.variable_str} → {self.tending_to_str}",
                        )
                        # Use sympy.limit directly on the L'Hopital expression
                        limit_result_lhopital = sympy.limit(
                            lhopital_expr, self.sympy_var, self.sympy_limit_point
                        )

                        formatted_lhopital_res = self._format_sympy_result(
                            limit_result_lhopital
                        )
                        self._add_step(
                            "L'Hôpital's Result",
                            f"Result after applying L'Hôpital's Rule: {formatted_lhopital_res}",
                        )

                        # Check if L'Hopital gave a conclusive answer
                        # Check for unevaluated Limit object or NaN
                        is_lhopital_conclusive = not (
                            isinstance(limit_result_lhopital, sympy.Limit)
                            or limit_result_lhopital is sympy.nan
                            or limit_result_lhopital is sympy.zoo
                        )

                        if is_lhopital_conclusive:
                            final_result_obj = limit_result_lhopital
                            limit_calculated = True
                            self._add_step(
                                "Conclusion",
                                f"L'Hôpital's Rule yields the limit: {formatted_lhopital_res}",
                            )
                        else:
                            self._add_step(
                                "L'Hôpital's Rule Inconclusive",
                                "L'Hôpital's Rule did not yield a determinate limit. May need further application or other methods.",
                            )

                    except Exception as e:
                        self._add_step(
                            "L'Hôpital's Rule Error",
                            f"Error applying L'Hôpital's Rule: {e}",
                        )
            ...
            if not limit_calculated:
                self._add_step(
                    "Attempt General Symbolic Limit",
                    f"Using sympy.limit on the expression: {format_expr_latex(current_expr)}",
                )
                try:
                    limit_result_sympy = sympy.limit(
                        current_expr, self.sympy_var, self.sympy_limit_point
                    )
                    formatted_sympy_res = self._format_sympy_result(limit_result_sympy)
                    self._add_step(
                        "Sympy Limit Result",
                        f"sympy.limit result: {formatted_sympy_res}",
                    )

                    # Check if sympy could evaluate it (not unevaluated Limit or NaN/zoo)
                    is_sympy_limit_conclusive = not (
                        isinstance(limit_result_sympy, sympy.Limit)
                        or limit_result_sympy is sympy.nan
                        or limit_result_sympy is sympy.zoo
                    )

                    if is_sympy_limit_conclusive:
                        final_result_obj = limit_result_sympy
                        limit_calculated = True
                        self._add_step(
                            "Conclusion",
                            f"General symbolic calculation yields: {formatted_sympy_res}",
                        )
                    else:
                        self._add_step(
                            "Symbolic Limit Inconclusive",
                            "sympy.limit returned an unevaluated or indeterminate result.",
                        )

                except NotImplementedError as e:
                    self._add_step(
                        "Symbolic Limit Error",
                        f"Sympy does not support calculating this limit type: {e}",
                    )
                    ...
                    self._add_step(
                        "Symbolic Limit Error",
                        f"An error occurred during sympy.limit: {e}",
                    )
                ...
                if self.fallback_tokens and self.numerical_limit_point is not None:
                    ...
                    if num_limit_point == INF:
                        ...
                        if val == INF:
                            ...
                            # Try formatting numerical result nicely
                            limit_approx_str = f"≈ {self._format_sympy_result(val)}"
                        ...
                        try:
                            left_val = self._safe_numerical_evaluate(
                                num_limit_point - self.epsilon
                            )
                            right_val = self._safe_numerical_evaluate(
                                num_limit_point + self.epsilon
                            )
                            self._add_step(
                            ...
                            if math.isnan(left_val) or math.isnan(right_val):
                                limit_approx_str = (
                                    "DNE (Numerical evaluation failed near point)"
                                )
                            ...
                            # Use relative tolerance check, especially important near zero
                            elif abs(left_val - right_val) < tolerance * (
                                1 + abs(left_val) + abs(right_val)
                            ):
                                ...
                                # Use the same formatting logic as symbolic results
                                limit_approx_str = (
                                    f"≈ {self._format_sympy_result(limit_approx)}"
                                )

                                self._add_step(
                                    "Numerical Result",
                                    f"Left/Right values are close. Approximated limit {limit_approx_str}",
                                )
                            else:
                                limit_approx_str = "DNE (Left/Right limits differ)"
                                self._add_step(
                                    "Numerical Result",
                                    f"Left ({left_val}) and Right ({right_val}) evaluations differ significantly.",
                                )
                    ...
                    if (
                        "DNE" not in self.result
                        and "Error" not in self.result
                        and "∞" not in self.result
                        and "Could not" not in self.result
                    ):
                        self._add_step(
                            "Conclusion",
                            f"Using numerical approximation as fallback: {self.result}",
                        )
                    else:
                        self._add_step(
                            "Conclusion", f"Numerical fallback result: {self.result}"
                        )
                    ...
                    self._add_step(
                        "Fallback Failed",
                        "Numerical fallback could not be attempted (initialization failed or limit point unsuitable).",
                    )
                    # Ensure result reflects failure if symbolic also failed
            ...
            if limit_calculated and final_result_obj is not None:
                ...
                not limit_calculated and self.result == "Could not determine limit"
            ):  # If numerical fallback didn't run or failed indeterminately
                # Check if a specific reason was logged, otherwise add a generic failure message
                found_reason = any(
                    step["step"] == "Symbolic Limit Inconclusive"
                    or "DNE" in step["explanation"]
                    or "Error" in step["explanation"]
                    or "failed" in step["explanation"].lower()
                    or "Inconclusive" in step["step"]
                    or "unavailable"
                    in self.result  # Check if numerical was unavailable
                    for step in self.steps
                )
                if not found_reason:
                    # Check if the final sympy object was an unevaluated limit or NaN
                    if isinstance(final_result_obj, sympy.Limit):
                        self.result = self._format_sympy_result(
                            final_result_obj
                        )  # Show unevaluated limit
                        self._add_step(
                            "Conclusion",
                            f"The limit could not be evaluated: {self.result}",
                        )
                    elif final_result_obj is sympy.nan:
                        self.result = "NaN (Indeterminate)"
                        self._add_step(
                            "Conclusion", "The limit is indeterminate (NaN)."
                        )
                    else:
                        self._add_step(
                            "Conclusion",
                            "Failed to determine the limit using available symbolic and numerical methods.",
                        )
                        self.result = "Could not determine limit"
        ...
        # Final check on the result string format
        if (
            isinstance(self.result, str)
            and self.result.startswith("$$")
            and self.result.endswith("$$")
        ):
            # Avoid double wrapping if _format_sympy_result returned LaTeX
            pass
        elif (
            final_result_obj is not None and not limit_calculated
        ):  # If calculation stopped but we had an object
            # Ensure the final result uses the standard formatter if calculation didn't formally conclude
            self.result = self._format_sympy_result(final_result_obj)
...
async def calculate_limit_endpoint(request: LimitRequest):
    ...
    try:
        ...
        if not request.expression or not request.variable or not request.tending_to:
            raise ValueError(
                "Expression, variable, and tending_to fields are required."
            )
        ...
        if re.search(
            ...
            raise ValueError(
                "Input contains potentially unsafe or unsupported characters."
            )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during limit calculation."
        )
